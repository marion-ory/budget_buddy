...
class OperationDialog(ctk.CTkToplevel):
    ...
    def valider(self):
        ...
        print(f"DEBUG valider: mt={mt}, desc={desc}, cat_nom={cat_nom}, categories_data={self.categories_data}")
        try:
            id_cat = next(c["ID"] for c in self.categories_data if c["Nom"] == cat_nom)
            if mt :
                desc = desc if desc else "Opération"
            ...
            print("DEBUG: Erreur catégorie — cat_nom introuvable dans categories_data")
...
class VirementInterneDialog(ctk.CTkToplevel):
    def __init__(self, master, comptes, callback):
        super().__init__(master)
        self.title("Virement Interne")
        self.geometry("350x450")
        self.callback = callback
        self.comptes = comptes  # liste des objets CompteBancaires
        self.configure(fg_color=BG_COLOR)
        self.attributes("-topmost", True)
        self.after(100, self.grab_set)

        # Noms des comptes pour les menus déroulants
        noms_comptes = [f"Compte {c.typecompte}" for c in self.comptes]

        ctk.CTkLabel(self, text="Virement Interne", font=FONT_TITLE).pack(pady=20)

        # Compte émetteur
        ctk.CTkLabel(self, text="Émetteur :", font=FONT_BODY, text_color=TEXT_GRAY).pack(pady=(10, 0))
        self.combo_emetteur = ctk.CTkOptionMenu(self, values=noms_comptes, width=220, fg_color=ACCENT_BLUE)
        self.combo_emetteur.pack(pady=5)

        # Compte destinataire
        ctk.CTkLabel(self, text="Destinataire :", font=FONT_BODY, text_color=TEXT_GRAY).pack(pady=(10, 0))
        self.combo_destinataire = ctk.CTkOptionMenu(self, values=noms_comptes, width=220, fg_color=ACCENT_BLUE)
        self.combo_destinataire.set(noms_comptes[1])  # Annexe par défaut
        self.combo_destinataire.pack(pady=5)

        # Montant
        self.entry_montant = ctk.CTkEntry(self, placeholder_text="Montant (€)", width=220)
        self.entry_montant.pack(pady=10)

        # Description
        self.entry_desc = ctk.CTkEntry(self, placeholder_text="Description (optionnel)", width=220)
        self.entry_desc.pack(pady=10)
    ...
    def valider(self):
        ...
        desc = self.entry_desc.get().strip() or "Virement interne"
        nom_src = self.combo_emetteur.get()
        nom_dest = self.combo_destinataire.get()
        ...
        if nom_src == nom_dest:
            ctk.CTkLabel(self, text="Émetteur et destinataire identiques", text_color="red").pack()
            return

        # On retrouve les objets compte correspondants
        print(f"DEBUG dialog: nom_src={nom_src}, nom_dest={nom_dest}")
        src = next(c for c in self.comptes if f"Compte {c.typecompte}" == nom_src)
        dest = next(c for c in self.comptes if f"Compte {c.typecompte}" == nom_dest)
        print(f"DEBUG dialog: src.typecompte={src.typecompte}, dest.typecompte={dest.typecompte}")
        self.callback(float(mt), desc, src, dest)
        self.destroy()
...
class PageHome(ctk.CTkFrame):
    ...
    def traiter_depot(self, mt, desc, id_cat):
        resultat = self.master.user_obj.comptes[0].effectuer_depot(float(mt), desc, id_cat)
        print(f"DEBUG depot: résultat={resultat}, solde après={self.master.user_obj.comptes[0].solde}")
        if resultat:
            self.refresh_data()
    ...
    def traiter_retrait(self, mt, desc, id_cat):
        resultat = self.master.user_obj.comptes[0].effectuer_retrait(float(mt), desc, id_cat)
        print(f"DEBUG retrait: résultat={resultat}, solde après={self.master.user_obj.comptes[0].solde}")
        if resultat:
            self.refresh_data()

    def ouvrir_virement(self):
        if len(self.master.user_obj.comptes) >= 2:
            VirementInterneDialog(
                self,
                self.master.user_obj.comptes,
                self.traiter_virement_interne
            )

    def traiter_virement_interne(self, mt, desc, src, dest):
        resultat = src.effectuer_transfert(mt, dest, desc, id_cat=None)
        print(f"DEBUG vir.interne: résultat={resultat}, solde src={src.solde}, solde dest={dest.solde}")
        if resultat:
            self.refresh_data()
    ...
    def traiter_virement_externe(self, nom, prenom, montant):
        ...
        print(f"DEBUG vir.externe: id_dest={id_dest}")
        if id_dest and self.master.user_obj.comptes[0].effectuer_transfert(
            float(montant), id_dest, f"Virement à {prenom} {nom}", 3
        ):
            print(f"DEBUG vir.externe: succès, solde après={self.master.user_obj.comptes[0].solde}")
            self.refresh_data()