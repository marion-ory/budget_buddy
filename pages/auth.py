...
from security import verifier_banquier
...
class PageLogin(ctk.CTkFrame):
    ...
    def valider_connexion(self):
        ...
        if self.master.login_user(email, mdp):
            role = self.master.user_obj.role

            if role == "Banquier":
                self.master.show_page(self.master.page_banquier)
            else:
...
class PageRegister(ctk.CTkFrame):
    def __init__(self, master):
        ...
        ctk.CTkLabel(
            ...
            text_color="gray",
            justify="center",
        ).pack(pady=(0, 10), padx=40, anchor="center")
    ...
    def valider_inscription(self):
        ...
        if not (nom and prenom and email and mdp):
            self.label_info.configure(text="Tous les champs sauf le code sont obligatoires", text_color="red")
            return

        if len(mdp) < 10:
            self.label_info.configure(text="Mot de passe trop court (10 caractères minimum)", text_color="red")
            return
        ...
        if inscription(nom, prenom, email, "", mdp, code):
            ...
            self.label_info.configure(text="Erreur : Cet email est déjà utilisé.", text_color="orange")
# Menu
