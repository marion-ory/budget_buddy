...
class Client(Users):
    ...
    def faire_virement(self, montant, compte_dest):
        print(
            f"Demande de virement de {montant}€ par le client {self.prenom} vers {compte_dest}"
        )
...
class Banquier(Users):
    ...
    def faire_virement(self, compte_emetteur, compte_dest, montant):
...
class Transactions:
    def __init__(
        self, id, description, montant, date, emetteur, beneficiaire, type, user_id
    ):
        self.id = id
    ...
    def __str__(self):
        return f(
            "Le {self.type} d'un {self.montant} en date du {self.date} depuis le compte {self.emetteur} vers {self.beneficiaire}"
        )
